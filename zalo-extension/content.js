...
(function () {
  ...
  function buildPanel() {
    ...
    panel.innerHTML = `
      ...
      <div class="zai-cfg" id="zai-cfg" style="display:none">
        ...
        <div id="zai-admin-section" style="display:none">
          <label style="color:#b45309">🔑 Gemini API Key (Admin — lưu 1 lần vào GAS)</label>
          <input id="zai-gemini-key" placeholder="AIza..." type="password">
          <div style="font-size:10px;color:#92400e;margin-bottom:6px">
            Key sẽ được lưu vào Settings sheet của GAS, dùng chung cho cả team. Không cần nhập lại.
          </div>
        </div>

        <div style="display:flex;gap:6px;align-items:center">
          <button class="zai-cfg-save" id="zai-cfg-save">💾 Lưu</button>
          <label style="font-size:11px;color:#6b7280;font-weight:normal;cursor:pointer">
            <input type="checkbox" id="zai-is-admin" style="margin-right:3px">Admin
          </label>
        </div>
        <div style="font-size:10px;color:#9ca3af;margin-top:6px">
          Nhân viên chỉ cần nhập URL GAS rồi Lưu. Admin thêm Gemini Key 1 lần cho cả team.
        </div>
      </div>
    ...
    document.getElementById('zai-is-admin').onchange = function () {
      _isAdmin = this.checked;
      document.getElementById('zai-admin-section').style.display = _isAdmin ? 'block' : 'none';
    };
  ...
  async function saveConfig() {
    ...
    if (!geminiKey || !_isAdmin) showMsg('zai-save-status', '✓ Đã lưu cài đặt', 2000);
  }
  ...
  window._zaiLookup = async function () {
    ...
    } catch (e) { area.innerHTML = ''; showError(e.message); }
  };
  ...
  window._zaiSaveStatus = async function () {
    ...
    try {
      ...
      const row  = {
        ...
        schedules: care.schedules||'', schedGoi: care.schedGoi||'', schedGoiNote: care.schedGoiNote||'',
        schedSP: care.schedSP||'', schedSPNote: care.schedSPNote||'', schedCS: care.schedCS||'',
        schedCSNote: care.schedCSNote||'', schedHen: care.schedHen||'', schedHenNote: care.schedHenNote||''
      };
      const res = await fetch(GAS_URL, {
        method: 'POST', body: JSON.stringify({ action: 'saveSingle', row }),
        headers: { 'Content-Type': 'text/plain' }
      });
  ...
  window._zaiGenerate = async function () {
    ...
    try {
      const res = await fetch(GAS_URL, {
        method: 'POST', body: JSON.stringify({ action: 'ai', prompt }),
        headers: { 'Content-Type': 'text/plain' }
      });
      ...
      sugArea.innerHTML = '';
      showError('Lỗi: ' + e.message + (e.message.includes('geminiKey') ? ' — Admin cần cài Gemini Key qua ⚙.' : ''));
    } finally { btn.disabled = false; btn.textContent = '✨ Tạo gợi ý AI'; }
  };
